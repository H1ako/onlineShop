...
function FavouritesList({ amount = "all", setTotalCost = null }) {
  const { customer } = useCustomer()
  const [favourites, setFavourites] = useState([])

  const updateFavourites = () => {
    getFavourites(amount).then((data) => {
      ...
      {favourites.map((favourite) => (
        <li key={favourite.id} className="favourites-list__favourite">
          <div className="favourite__info">
            <h3 className="info__product-creator">
              {favourite.product?.author} - {favourite.product?.brand}
            </h3>
            <ProductPrice
          ...
          <div className="favourite__product-info">
            <Link
              to={`/products/${favourite.product?.id}`}
          ...
          <img
            ...
            alt=""
            className="favourite__product-pic"
          />
        </li>
      ))}
    </ul>
